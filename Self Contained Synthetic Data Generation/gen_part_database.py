...
scene.cycles.samples = 1200 # Decreasing number of samples from 4096 to increase render times
...
render_path = base_path + "Renders/Parts_Database/"
        ...
        if(model.endswith('.dat')):
            ...
            if not os.path.isfile(fileName):
                ...
                for iteration in range(num_generate):
                    ...
                    renderPiece(render_path, iteration)
                removeModel()
                numTest += 1
        else:
            continue
...
def renderPiece(final_path, num):
    ...
    bpy.context.scene.render.filepath = os.path.join(final_path, nameCut) #to get unique file names will have to fix this one
...
def camera_view_bounds_2d(scene, cam_ob, me_ob):
    ...
    # # For this function, will join and then separate the two meshes
    ...
    # # Separate the meshes if a part that has multiple parts:
    # # NOTE: IF WE ARE GETTING ERRORS. HOPEFULLY THERE ARE NOT MULTIPART PIECES THAT ARE MADE OF MULTIPLE MATERIALS.
    # # THAT WOULD SUCK. SHOULD DEFINITELY TRY TO FUTURE PROOF THIS. MAYBE SEPARATE BY MATERIAL IS NOT RIGHT? NOT SURE.
    # if(multiPart):
    #     bpy.ops.object.select_all(action='DESELECT')
...
def getMeshes(model):
    ...
    #             allMeshObjects += getMeshes(children)
    #             continue
    #         allMeshObjects.append(children)
    # else:
    #     allMeshObjects.append(model)
...
def importModel(model_path):
    ...
    #joinMeshes(bpy.data.objects[current_name])
...
def main():
    setUpScene()
    bpy.data.objects["paper"].hide_render = True
    execute()
